...
import { Eye, LoaderCircle, Sparkles } from "lucide-react";
...
export function ObservationWorkspace() {
  ...
  const currentPayload = useMemo(() => JSON.stringify(sendInput), [sendInput]);
  ...
  function showPreview() {
    const replacementLines = anonymized.replacements.map((item) => `${item.original} → ${item.replacement}`);
    const payload = JSON.stringify(sendInput);
    setPreviewPayload(payload);
    setPreview(`${replacementLines.length ? `이름 치환\n${replacementLines.join("\n")}\n\n` : ""}마스킹된 항목: ${anonymized.maskedTypes.join(", ") || "없음"}\n\n실제 AI 전송 JSON\n${JSON.stringify(sendInput, null, 2)}`);
  }

  async function submit(event: FormEvent) {
    event.preventDefault();
    if (!preview || previewPayload !== currentPayload || !privacyConfirmed) { setError("현재 전송 내용을 익명화 미리보기에서 확인한 뒤 개인정보 확인에 동의해 주세요."); return; }
    ...
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] lg:items-start">
        <form onSubmit={submit} className="grid gap-5">
          <PrivacyNotice />
          <FormSection title="관찰 기본 정보">
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="연령" htmlFor="age" required><SelectInput id="age" value={form.age} onChange={(event) => updateForm("age", event.target.value as FormState["age"])}><option>만 3세</option><option>만 4세</option><option>만 5세</option></SelectInput></Field>
              <Field label="관찰 날짜" htmlFor="observation-date"><TextInput id="observation-date" type="date" value={form.observationDate} onChange={(event) => updateForm("observationDate", event.target.value)} /></Field>
              <Field label="놀이 이름" htmlFor="play-name" required><TextInput id="play-name" value={form.playName} onChange={(event) => updateForm("playName", event.target.value)} maxLength={120} required placeholder="예: 가을 택배소" /></Field>
          ...
          <FormSection title="교사가 관찰한 장면" description="해석보다 실제 행동과 들은 말을 먼저 적어 주세요.">
            <Field label="관찰 메모" htmlFor="memo" required hint={`${form.memo.length.toLocaleString()} / 6,000자`}><TextArea id="memo" value={form.memo} onChange={(event) => updateForm("memo", event.target.value)} maxLength={6000} required className="min-h-48" placeholder="유아가 무엇을 했고 어떤 말을 했는지 적어 주세요." /></Field>
            <Field label="직접 들은 말" htmlFor="quotes"><TextArea id="quotes" value={form.directQuotes} onChange={(event) => updateForm("directQuotes", event.target.value)} maxLength={4000} placeholder="실제로 들은 표현만 적어 주세요." /></Field>
          ...
          <FormSection title="전송 전 익명화">
            <Field label="익명화할 실제 이름 목록" htmlFor="names" hint="쉼표나 줄바꿈으로 구분합니다. 이 목록은 브라우저 저장소나 서버에 저장하지 않습니다."><TextArea id="names" value={form.names} onChange={(event) => updateForm("names", event.target.value)} placeholder="예: 민준, 지우, 서연" className="min-h-20" /></Field>
            <button type="button" onClick={showPreview} className="action-button w-fit"><Eye size={17} aria-hidden="true" /> 익명화 결과 확인</button>
            {preview && <pre className="max-h-80 overflow-auto whitespace-pre-wrap rounded-xl bg-[#f3f6f3] p-4 text-sm leading-6" aria-label="AI 전송 내용 미리보기">{preview}</pre>}
            <label className="flex cursor-pointer gap-3 rounded-xl border border-[var(--line)] bg-white p-4 text-sm leading-6"><input type="checkbox" checked={privacyConfirmed} disabled={!preview || previewPayload !== currentPayload} onChange={(event) => setPrivacyConfirmed(event.target.checked)} className="mt-1 size-4 accent-[var(--sage)] disabled:opacity-50" /><span>위 전송 내용을 확인했고, AI 초안을 원문과 비교해 검토하겠습니다.</span></label>
          </FormSection>
          <FormSection title="결과 설정">
            <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="no-print flex flex-wrap gap-3">
            ...
            <button type="button" onClick={() => { setForm(sample); setPreview(""); setPreviewPayload(""); setPrivacyConfirmed(false); }} className="action-button">가을 택배소 샘플</button>
          </div>
          ...
          <DraftManager kind="observation" suggestedName={form.playName || "놀이 기록"} data={safeDraftData} onLoad={(value) => { const restored = value as { form: Partial<FormState>; result: ObservationResult | null; mode: GenerationMode; document?: unknown; resultContext?: ResultContext }; const restoredForm = { ...initialForm, ...restored.form, names: "" }; setForm(restoredForm); setResult(restored.result); setMode(restored.mode); setDocumentState(parseEditableDocumentState(restored.document)); setResultContext(restored.resultContext || (restored.result ? { playName: restoredForm.playName, age: restoredForm.age } : null)); setDocumentRevision((revision) => revision + 1); setPreview(""); setPreviewPayload(""); setPrivacyConfirmed(false); }} />
        </form>

        <div className="lg:sticky lg:top-5">
          {result ? <><EditableDocument key={`document-${documentRevision}`} title={settings.customDocumentTitle || result.title} sections={sections} mode={mode} prefix="놀이기록" initialDocument={documentState} onDocumentChange={setDocumentState} /><PlaySupportImageGenerator key={`image-${documentRevision}`} playName={resultContext?.playName || form.playName} age={resultContext?.age || form.age} supportSummary={[...result.playSupport.environment, ...result.playSupport.materials].join(", ")} /></> : <div className="grid min-h-[32rem] place-items-center rounded-[2rem] border border-dashed border-[#b8c8be] bg-[rgba(255,253,248,0.7)] p-8 text-center"><div><Sparkles className="mx-auto text-[var(--sage)]" size={34} aria-hidden="true" /><h2 className="mt-4 text-xl font-black">아직 만든 초안이 없습니다</h2><p className="mt-2 max-w-sm leading-7 text-[var(--muted)]">입력한 내용을 바탕으로 객관적 기록, 학부모용 이야기, 다음 놀이 지원과 교사 확인 항목을 만듭니다.</p></div></div>}
        </div>
      </div>
    </div>
  );
