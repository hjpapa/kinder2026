    ...
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-2 lg:items-start">
        <form onSubmit={submit} className="grid gap-5">
          ...
          <FormSection title="무엇을 알려드릴까요?" description="학부모에게 전할 내용을 한 번에 적어 주세요. 날짜·장소·준비물도 메모 안에 함께 적으면 됩니다.">
            <Field label="알려줄 내용" htmlFor="event-summary" required hint={`${form.eventSummary.length.toLocaleString()} / 8,000자`}><TextArea id="event-summary" value={form.eventSummary} onChange={(event) => update("eventSummary", event.target.value)} required maxLength={8000} className="min-h-56" placeholder="예: 9월 18일 오전에 강당에서 전통놀이 한마당을 엽니다. 편한 옷을 입혀 보내 주세요. 비가 와도 강당에서 진행합니다." /></Field>
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="행사명 (선택)" htmlFor="notice-event-name"><TextInput id="notice-event-name" value={form.eventName} onChange={(event) => update("eventName", event.target.value)} placeholder="비우면 안내문 제목으로 정리" /></Field>
              <Field label="문체 (선택)" htmlFor="notice-tone"><SelectInput id="notice-tone" value={form.tone} onChange={(event) => update("tone", event.target.value as NoticeForm["tone"])}><option>따뜻하게</option><option>담백하게</option><option>공식적으로</option></SelectInput></Field>
            </div>
          </FormSection>
          <details className="rounded-2xl border border-[var(--line)] bg-white p-4">
            <summary className="cursor-pointer font-black text-[var(--sage-dark)]">대상과 확정 정보 따로 입력하기</summary>
            <p className="mt-2 text-sm leading-6 text-[var(--muted)]">위 메모에 이미 적은 내용은 다시 입력하지 않아도 됩니다.</p>
            <div className="mt-5 grid gap-5">
              <Field label="받는 대상" htmlFor="notice-audience"><TextInput id="notice-audience" value={form.audience} onChange={(event) => update("audience", event.target.value)} /></Field>
              <Field label="확정된 날짜·장소·준비물" htmlFor="confirmed-details" hint="확정된 내용만 적어 주세요. 비어 있으면 확인 항목으로 남깁니다."><TextArea id="confirmed-details" value={form.confirmedDetails} onChange={(event) => update("confirmedDetails", event.target.value)} /></Field>
            </div>
          </details>
          <label className="flex cursor-pointer gap-3 rounded-xl border border-[var(--line)] bg-white p-4 text-sm leading-6"><input type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} className="mt-1 size-4 accent-[var(--sage)]" /><span>발송 전에 일시·장소·준비물·사진 촬영·문의 방법을 기관 정보와 대조하겠습니다.</span></label>
          <div className="no-print flex gap-3"><button disabled={loading} className="inline-flex min-h-14 flex-1 items-center justify-center gap-2 rounded-2xl bg-[var(--sage-dark)] px-6 text-lg font-black text-white disabled:opacity-60">{loading ? <LoaderCircle className="animate-spin" aria-hidden="true" /> : <Sparkles aria-hidden="true" />} {loading ? "학부모 안내 내용을 정리하고 있습니다…" : "가정통신문 초안 만들기"}</button>{loading && <button type="button" onClick={() => controller.current?.abort()} className="action-button">취소</button>}</div>
          ...
          <DraftManager kind="parent-notice" suggestedName={form.eventName || "가정통신문"} data={draftData} onLoad={(value) => { const restored = value as { form: Partial<NoticeForm>; result: ParentNoticeResult | null; mode: GenerationMode; document?: unknown }; setForm({ ...initialForm, ...restored.form }); setResult(restored.result); setMode(restored.mode); setDocumentState(parseEditableDocumentState(restored.document)); setDocumentRevision((revision) => revision + 1); setConfirmed(false); }} />
        </form>
        <div className="lg:sticky lg:top-5">
          {result ? <EditableDocument key={`document-${documentRevision}`} title={settings.customDocumentTitle || result.title} sections={sections} mode={mode} prefix="가정통신문" initialDocument={documentState} onDocumentChange={setDocumentState} /> : <div className="grid min-h-[30rem] place-items-center rounded-[2rem] border border-dashed border-[#b8c8be] bg-[rgba(255,253,248,0.7)] p-8 text-center"><div><Sparkles className="mx-auto text-[var(--sage)]" size={34} aria-hidden="true" /><h2 className="mt-4 text-xl font-black">알려줄 내용을 편하게 적어 주세요</h2><p className="mt-2 max-w-sm leading-7 text-[var(--muted)]">한 번의 메모를 학부모가 읽기 쉬운 안내문으로 정리하고, 확인할 정보는 따로 표시합니다.</p></div></div>}
        </div>
      </div>
    </div>
  );
